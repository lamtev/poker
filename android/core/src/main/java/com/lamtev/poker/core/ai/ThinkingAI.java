...
public final class ThinkingAI extends AbstractAI {
    ...
    private void onShowdown() {
        try {
            ...
            PokerHand betterHand = null;
            for (PokerHand it : handsOfPotConcurrents()) {
                if (it.compareTo(itsHand) > 0) {
                    betterHand = it;
                    break;
                }
            }
            if (betterHand != null && moveAbility.foldIsAble()) {
                poker().fold();
            } else {
                poker().showDown();
            }
    ...
    private List<PokerHand> handsOfPotConcurrents() {
        ...
        Set<Map.Entry<String, PokerHand>> entrySet = playersHands.entrySet();
        for (Map.Entry<String, PokerHand> entry : entrySet) {
            String id = entry.getKey();
            PokerHand hand = entry.getValue();
